...
{
	// Privates:
	Godot.Collections.Array units; // OK
	...
	//
	public List<BSUnit> factionUnits = new List<BSUnit>();

	
	public override void _Ready()
	{
		Dictionary _u = (Dictionary)ResourceLoader.Load<Json>("res://7oi8zeiqfamiur21.json").Data;
		...
		for (int j = 0; j < units.Count; j++)
		{
			...
			for(int k = 0; k < weaps.Count; k++)
			{
				...
				foreach(Dictionary sr in sp)
				{
					...
					else if((string)sr["name"] == "Deadly")
					{
						l.dmg = (int)sr["rating"];
						//GD.Print("deadly found");
					}
					else
					{
			...
			Godot.Collections.Array rules = (Godot.Collections.Array)ev["rules"]; // wtf where is speicalRules
			nu.heartsPerModel = 1;
			...
			{	// id, name, rating like above 
				...
				else if ((string)r["name"] == "Fast")
				{
					nu.move = 8; //GD.Print("Fast model found.");
				}
				else if ((string)r["name"] == "Slow")
				{
					nu.move = 4; //GD.Print("Slow model found.");
				}
				else
				{
					BSAbility ab = new BSAbility
